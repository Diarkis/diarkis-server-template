...
public class GetFieldInfo
{
	...
	public bool Unpack(byte[] bytes)
	{
		...
		/* long */
		byte[] fieldOfVisionSizeBytes = DiarkisPacket.Slice(bytes, offset, 8);
		Array.Reverse(fieldOfVisionSizeBytes);
		...
		/* long */
		byte[] fieldSizeBytes = DiarkisPacket.Slice(bytes, offset, 8);
		Array.Reverse(fieldSizeBytes);
		...
		/* int */
		byte[] nodeCountBytes = DiarkisPacket.Slice(bytes, offset, 4);
		Array.Reverse(nodeCountBytes);
