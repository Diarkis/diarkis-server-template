...
if [ $# != 3 ]; then
    echo "Usage: $0 DIARKIS_PROJECT_ID NAMESPACE AWS_ID"
    exit 1
fi
...
if [ -e ${CLIENT_DIR} ]; then
    ...
    cp -Rip ${DIR_BASE}/../overlays/base/aws ${CLIENT_DIR}
    ## SED NAMESPACE
    ...
    ## SED AWS ACCOUNT
    find ${CLIENT_DIR} -type f | xargs sed -i "" "s/__AWS_ACCOUNT_NUM__/${AWS_ID}/g"
fi