...
func run() int {
	var (
		...
		flagLatitude  = flag.Float64("latitude", 35.65877910898138, "the client's latitude")
		flagLongitude = flag.Float64("longitude", 139.70128360250285, "the client's longitude")
	)
	...
	cli := new(client)
	cli.uid = *flagUID
	cli.latitude = *flagLatitude
	cli.longitude = *flagLongitude
...
func (c *client) startMatching() error {
	...
	params.Latitude = c.latitude
	params.Longitude = c.longitude

	logger.Info("start matching with latitude:%f, longitude:%f", params.Latitude, params.Longitude)
